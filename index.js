...
// Create Failed Payments table in Airtable if it doesn't exist
async function ensureAirtableTable() {
  try {
    // First, try to create the table with the required fields
    const tableConfig = {
      name: AIRTABLE_TABLE_NAME,
      fields: [
        { name: 'Payment Intent ID', type: 'singleLineText' },
        { name: 'Customer Email', type: 'email' },
        { name: 'Amount', type: 'currency', options: { precision: 2 } },
        { name: 'Currency', type: 'singleLineText' },
        { name: 'Failure Code', type: 'singleLineText' },
        { name: 'Failure Message', type: 'multilineText' },
        { name: 'Failed At', type: 'dateTime' },
        { name: 'Customer ID', type: 'singleLineText' },
        { name: 'Status', type: 'singleSelect', options: { 
          choices: [
            { name: 'Failed', color: 'redBright' },
            { name: 'Notified', color: 'yellowBright' },
    ...
    addLog('info', 'Created Failed Payments table in Airtable');
  } catch (error) {
    if (error.response && error.response.status === 422) {
      addLog('info', 'Failed Payments table already exists in Airtable');
    } else {
      addLog('error', 'Error ensuring Airtable table', error.message);
    }
  }
}
...
async function logToAirtable(paymentData) {
  try {
    ...
    addLog('error', 'Error logging to Airtable', error.message);
    return false;
  }
}
...
app.post('/test', async (req, res) => {
  try {
    ...
    res.json({
      ...
      results: {
        airtable: 'table ensured',
        email: testEmailResult ? 'sent' : 'failed'
      },
      timestamp: new Date().toISOString()
    });
  } catch (error) {
...
async function initialize() {
  addLog('info', 'Starting Stripe Payment Monitor Agent');
  await ensureAirtableTable();
  addLog('info', 'Agent initialization complete');
}
